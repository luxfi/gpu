...
func (c *Context) Zeros(shape []int, dtype Dtype) *Array {
	...
	handle := C.mlx_zeros(&cShape[0], C.int(len(shape)), C.int(dtype))
...
func (c *Context) Ones(shape []int, dtype Dtype) *Array {
	...
	cShape := intsToCInts(shape)
	handle := C.mlx_ones(&cShape[0], C.int(len(shape)), C.int(dtype))
...
func (c *Context) Random(shape []int, dtype Dtype) *Array {
	...
	cShape := intsToCInts(shape)
	handle := C.mlx_random(&cShape[0], C.int(len(shape)), C.int(dtype))
...
func (c *Context) Sum(a *Array, axis ...int) *Array {
	...
	if len(axis) == 0 {
		...
		handle = C.mlx_sum(a.handle, &cAxis[0], C.int(len(axis)))
	}
...
func (c *Context) Mean(a *Array, axis ...int) *Array {
	...
	if len(axis) == 0 {
		...
		cAxis := intsToCInts(axis)
		handle = C.mlx_mean(a.handle, &cAxis[0], C.int(len(axis)))
	}
...
func (c *Context) FromSlice(data []float32, shape []int, dtype Dtype) *Array {
	...
	cShape := intsToCInts(shape)
	handle := C.mlx_from_slice(cData, C.int(len(data)), &cShape[0], C.int(len(shape)), C.int(dtype))
